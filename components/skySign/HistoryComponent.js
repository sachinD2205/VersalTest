...
const Index = (props) => {
  ...
  const getNewMarriageRegistractionHistoryDetails = () => {
    if (props.serviceId == 7) {
      axios
        .get(
          `${urls.SSLM}/trnIssuanceOfBusinessLicense/getByIdAndServiceId?id=${props.applicationId}&serviceId=${props.serviceId}`,
          {
            headers: {
        ...
        .then((resp) => {
          setTableData(
            resp?.data?.trnApplicantDetailsHistoryDao?.map((r, i) => {
              return {
                srNo: i + 1,
                ...r,
              };
    ...
    } else if (props.serviceId == 8) {
      axios
        .get(
          `${urls.SSLM}/trnIssuanceOfIndustrialLicense/getByServiceIdAndId?id=${props.applicationId}&serviceId=${props.serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${userToken}`,
            },
    ...
    } else if (props.serviceId == 9) {
      axios
        .get(
          `${urls.SSLM}/Trn/TrnIssuanceOfStoreLicense/getByIdAndServiceId?id=${props.applicationId}&serviceId=${props.serviceId}`,
          {
            headers: {
              Authorization: `Bearer ${userToken}`,
            },
