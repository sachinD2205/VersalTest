...
const Shift = () => {
  ...
  const getEquipment = (_pageSize = 10, _pageNo = 0) => {
    setLoading(true);
    axios
      // .get(`http://192.168.68.145:9006/pabbm/api/mstEventHour/getAll`, {
        ...
        let _res = result.map((val, i) => {
          return {
            ...
            auditoriumName: val?.auditoriumKey
              ...
              : "-",
            eventKey: val?.eventName
              ? events?.find((obj) => {
                  return obj?.id == Number(val?.eventName);
                })?.eventNameEn
              : "-",
            eventHour: val?.eventHour ? val?.eventHour : "-",
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.PABBMURL}/mstEventHour/save`, body, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
            })
  ...
  const onSubmitForm = (formData) => {
    ...
    const finalBodyForApi = {
      ...
      id: formData.id,
      eventHour: Number(formData.eventHours),
      auditoriumKey: formData.auditoriumName,
      eventName: formData.eventKey,
    ...
    axios
      .post(`${urls.PABBMURL}/mstEventHour/save`, finalBodyForApi, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      // axios
      //   .post(
      //     `http://192.168.68.145:9006/pabbm/api/mstEventHour/save`,
      //     finalBodyForApi
      //   )
      .then((res) => {
        if (res.status == 201) {
          ...
          <>
            <IconButton
              onClick={() => {
                ...
                reset(params.row);
                setValue("auditoriumName", params.row.auditoriumKey);
                setValue("eventKey", params.row.eventName);

                setValue("startTime", params.row._startTime);
                setValue("endTime", params.row._endTime);
                setValue("eventHours", params.row.eventHour);
              }}
            >
  ...
  return (
    <div>
      <Paper style={{ marginTop: "10%" }}>
        ...
        <PabbmHeader labelName="shiftTitle" />
        {isOpenCollapse && (
                          ...
                          <TimePicker
                            label={
                              <span style={{ fontSize: 16 }}>
                                <FormattedLabel id="startTime" required />
                              </span>
                            }
                        ...
                        <LocalizationProvider dateAdapter={AdapterMoment}>
                          <TimePicker
                            label={
                              <span style={{ fontSize: 16 }}>
                                {" "}
                                <FormattedLabel id="endTime" required />
                              </span>
                            }
                            value={field.value || null}
                            onChange={(time) => {
              ...
              <Grid container sx={{ padding: "10px" }}>
                ...
                >
                  <TextField
                    ...
                    id="outlined-basic"
                    InputLabelProps={{
                      shrink: watch("endTime") ? true : false,
                    }}
                    label={<FormattedLabel id="eventHours" required />}
                    variant="outlined"
                    {...register("eventHours")}
                  />
                </Grid>
                ...
                >
                  <TextField
                    ...
                    style={{ backgroundColor: "white" }}
                    id="outlined-basic"
                    label={<FormattedLabel id="timeSlot" />}
                    variant="outlined"
                    InputLabelProps={{
                      shrink: watch("endTime") ? true : false,
                    }}
                    ...
                    helperText={
                      errors?.timeSlot ? errors.timeSlot.message : null
                    }
                  />
                </Grid>
              </Grid>
