...
function BookingCancellation() {
  ...
  const onSubmitFunc = (data) => {
    ...
    if (watch("fromDate") && watch("toDate")) {
      ...
      axios
        .get(
          `${urls.PABBMURL}/trnBookingCancellationProcess/getCancellationReportByDateAndAuditoriumId`,
          {
            params: {
              fromDate: fromDate,
              toDate: toDate,
        ...
        .then((res) => {
          ...
          if (res?.status === 200 || res?.status === 201) {
            if (res?.data?.trnBookingCancellationProcessList?.length > 0) {
              let _res = res?.data?.trnBookingCancellationProcessList?.map(
                (r, i) => {
                  ...
                  return {
                    ...
                    // boardChargesAmount:
                    //   r?.trnAuditoriumBookingOnlineProcess?.boardChargesAmount,
                    // securityGuardChargeAmount:
  ...
  return (
    <Paper>
      ...
      <PabbmHeader
        language={language}
        enName="Booking Cancellation Report"
        mrName="बुकिंग रद्द करण्याचा अहवाल"
      />
