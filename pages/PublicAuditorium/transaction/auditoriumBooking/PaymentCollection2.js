...
const Index = (props) => {
  ...
  const getAuditoriumEvents = () => {
    axios
      ...
      .then((r) => {
        setEvents(r?.data?.trnAuditoriumEventsList);
      })
      ?.catch((err) => {
        console.log("err", err);
  ...
  const handlePay = () => {
    ...
    console.log(
      "showData",
      showData?.applicationNumber,
      typeof showData?.applicationNumber
    );
    ...
    sweetAlert({
      ...
      if (willDelete) {
        ...
        let customObject = {
          applicationNumber: showData?.applicationNumber,
          applicationDate: showData?.applicationDate,
          applicantName: showData?.applicantName,
          serviceId: showData?.serviceId,
          depositAmount: showData?.depositAmount,
          applicantFlatHouseNo: showData?.applicantFlatHouseNo,
          applicantFlatBuildingName: showData?.applicantFlatBuildingName,
          applicantArea: showData?.applicantArea,
          applicantLandmark: showData?.applicantLandmark,
          applicantPinCode: showData?.applicantPinCode,
          applicantCountry: showData?.applicantCountry,
          applicantState: showData?.applicantState,
          applicantCity: showData?.applicantCity,
          timeSlotList:
            showData?.timeSlotList && JSON.parse(showData?.timeSlotList),
          eventDate: showData?.eventDate,
          eventDetails: showData?.eventDetails,
          eventKey: events?.find((obj) => obj.id == showData?.eventKey)
            ?.eventNameEn,
          eventTitle: showData?.eventTitle,
          auditoriumName: auditoriums?.find((obj) => {
            return obj?.id == showData?.auditoriumId;
          })?.auditoriumNameEn,
        };
        ...
        let testBody = {
          ...
          amount:
            router?.query?.data &&
            JSON.parse(router?.query?.data)?.statusId == "APPLICATION_DRAFT"
              ? showData?.depositAmount
              : showData?.rentAmount,
          divertPageLink:
            // "/PublicAuditorium/transaction/auditoriumBooking/pgSuccess",
            router?.query?.data &&
            JSON.parse(router?.query?.data)?.statusId == "APPLICATION_DRAFT"
          ...
          trnOtherInfo:
            router?.query?.data &&
            JSON.parse(router?.query?.data)?.statusId == "APPLICATION_DRAFT"
              ? showData && JSON.stringify(obj)
              : JSON.stringify(showData),
          // applicationNo: showData && JSON.stringify(obj),
          applicationNo: JSON.stringify(customObject),
        ...
        console.log("dumbObjectBody", dumbObjectBody);
        ...
        if (watch("payment.paymentType") == 2) {
          ...
          JSON.parse(router?.query?.data)?.statusId == "APPLICATION_DRAFT"
            ? axios
                .post(
                  // `http://192.168.68.181:8090/cfc/payment/transaction/paymentCollection/initiatePaymentV1`,
            ...
            .then((res) => {
              ...
              swal(
                ...
                language == "en"
                  ...
                  : `पेमेंट यशस्वीरित्या गोळा केले! पावती क्रमांक आहे - ${
                      res?.data?.message?.split("$")[1]
                    }`,
                "success"
              );
  ...
  // Convert the rupee part to words
  const rupeesWordsDeposit =
    showData && converter.toWords(Math.floor(showData?.depositAmount));

  // Convert the paisa part to words
  const paisaWordsDeposit =
    showData &&
    converter.toWords(
      Math.round(
        (showData?.depositAmount - Math.floor(showData?.depositAmount)) * 100
      ),
      { unit: "paisa" }
    );
          ...
          <div className={styles.appDetails}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <h4>
                ...
                <b>
                  {showData?.applicationDate &&
                    moment(showData?.applicationDate, "YYYY-MM-DD").format(
                      "DD-MM-YYYY"
                    )}
                </b>
              </h4>
            ...
            <table className={styles.__table}>
              ...
              {router?.query?.payment == "deposit" ? (
                <tr className={styles.__tr}>
                  <td className={styles.__td}>1)</td>
                  <td className={styles.__td}>Deposit Amount / अनामत रक्कम</td>
                  <td className={styles.__tdAmt}>
                    <b>{showData?.depositAmount?.toFixed(2)}</b>
                  </td>
                </tr>
              ) : (
                <>
                  <tr className={styles.__tr}>
                    ...
                    <td className={styles.__tdAmt}>
                      <b>{(showData?.rentAmount / 1.18)?.toFixed(2)}</b>
                    </td>
                  </tr>
                  <tr className={styles.__tr}>
                    ...
                    <td className={styles.__tdAmt}>
                      <b>
                        {(
                          (showData?.rentAmount - showData?.rentAmount / 1.18) /
                          2
                        )?.toFixed(2)}
                      </b>
                    </td>
              ...
              <tr className={styles.__tr}>
                ...
                <td className={styles.__tdAmt}>
                  <b>
                    ...
                    {router?.query?.payment == "deposit"
                      ? (showData && showData?.depositAmount)?.toFixed(2)
                      : `${showData && showData?.rentAmount?.toFixed(2)} रु`}
                  </b>
                </td>
              </tr>
              <tr className={styles.__tr}>
                ...
                <td className={styles.__tdAmt}>
                  <b>
                    ...
                    <span style={{ textTransform: "capitalize" }}>
                      {router?.query?.payment == "deposit"
                        ? showData &&
                          `${rupeesWordsDeposit} rupees and ${paisaWordsDeposit} paise`
                        : // `${converter.toWords(showData?.depositAmount)} Only`
                          showData &&
                          // `${converter.toWords(showData?.rentAmount)} Only`
            ...
            >
              ...
              <h4 style={{ marginLeft: "10px" }}>
                <b>
                  {
                    auditoriums?.find((obj) => obj.id == showData?.auditoriumId)
                      ?.auditoriumNameEn
                  }
                </b>
              </h4>
            ...
            <div>
              <div className={styles.row4}>
                <div>
                  ...
                  >
                    {router?.query?.data &&
                    JSON.parse(router?.query?.data)?.statusId ==
                      "APPLICATION_DRAFT" ? (
