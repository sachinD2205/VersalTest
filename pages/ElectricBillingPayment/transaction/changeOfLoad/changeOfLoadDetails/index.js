...
const Index = () => {
  ...
  const handleViewActions = (paramsRow) => {
    if (paramsRow.status === "DG Waiting for Dy Eng") {
      router.push({
        pathname:
          "/ElectricBillingPayment/transaction/changeOfLoad/viewChangeOfLoad",
        query: {
          id: paramsRow.id,
        },
      });
  ...
  // billingDivisionAndUnit, consumptionType
  ...
  const getNewConnectionsData = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      ...
      .then((r) => {
        ...
        if (department && consumptionType && billingDivisionAndUnit) {
            ...
            <Tooltip title="Edit">
              <IconButton
                onClick={() => {
                  router.push({
                    pathname:
                      "/ElectricBillingPayment/transaction/changeOfLoad/addChangeOfLoad",
                    query: {
                      id: params.row.id,
                    },
                  });
