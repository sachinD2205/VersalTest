...
import schema from "../../../../../containers/schema/ElelctricBillingPaymentSchema/changeOfMeterSchema";
import {
  DatePicker,
  DesktopDatePicker,
...
const Index = () => {
  ...
  const onSubmitForm = (formData) => {
    ...
    let _formData = {
      ...dataSource,
      oldmsedclCategoryKey: formData?.oldmsedclCategoryKey,
      oldMeterConnectionDate: formData?.oldMeterConnectionDate,
      oldMeterInitialReading: formData?.oldMeterInitialReading,
      oldMeterReadingDate: formData?.oldMeterReadingDate,
      oldMeterNo: formData?.oldMeterNo,
      oldMeterStatusKey: formData?.oldMeterStatusKey,

      msedclCategoryKey: formData?.msedclCategoryKey,
      meterConnectionDate: formData?.meterConnectionDate,
      ...
      meterNo: formData?.newMeterNo,
      meterStatusKey: formData?.newMeterStatusKey,
      meterReadingDate: formData?.meterReadingDate,
      docForChangeOfMeter: referalDocument,
      reasonForChangeOfMeter: formData?.reasonForChange,
    };
    const tempData = axios
      ...
      .then((res) => {
        if (res.status == 201) {
          setLoading(false);
          sweetAlert(
            language === "en" ? "Changed!" : "बदलले",
            language === "en"
              ? "Meter Changed Successfully !"
              : "मीटर यशस्वीरित्या बदलले!",
            "success",
            { button: language === "en" ? "Ok" : "ठीक आहे" }
          );
          router.push(
  ...
  const discard = async (e) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(`${urls.CFCURL}/file/discard?filePath=${referalDocument}`)
          .then((res) => {
            if (res.data.status == "SUCCESS") {
              setReferalDocument(null);
  ...
  const handleClearButton = (e) => {
    setValue("msedclCategoryKey", null);
    setValue("meterConnectionDate", null);
    setValue("oldMeterClosingDate", null);
    setValue("oldMeterLastReading", null);
    setValue("newMeterInstallationDate", null);
    setValue("newMeterInitialReading", null);
    setValue("newMeterNo", null);
    setValue("newMeterStatusKey", null);
    setValue("meterReadingDate", null);
    setValue("reasonForChange", null);
    referalDocument ? discard(e) : "";
  };
                    ...
                    <Grid container spacing={2} sx={{ padding: "1rem" }}>
                      ...
                      >
                        <TextField
                          ...
                          id="standard-textarea"
                          label={
                            <FormattedLabel id="oldMeterLastReading" required />
                          }
                          variant="standard"
                      ...
                      >
                        <TextField
                          ...
                          name="newMeterNo"
                          id="standard-textarea"
                          label={<FormattedLabel id="newMeterNo" required />}
                          variant="standard"
                          // value={newMeterNo}
                          {...register("newMeterNo")}
                      ...
                      >
                        ...
                        >
                          <InputLabel id="demo-simple-select-standard-label">
                            <FormattedLabel id="newMeterStatus" required />
                          </InputLabel>
                          <Controller
                            render={({ field }) => (
                              <Select
