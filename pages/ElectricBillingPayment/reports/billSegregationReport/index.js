...
const Index = () => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Bill Segregation Report",
  });
    ...
    if (reportData != null && reportData != undefined) {
      setData(
        reportData?.map((r, i) => ({
          ...
          division: !divisionDropDown?.find((obj) => {
            return obj.id == r?.divisionKey;
          })
            ? "-"
            : divisionDropDown.find((obj) => {
                return obj.id == r?.divisionKey;
              }).division,
          divisionMr: !divisionDropDown?.find((obj) => {
            return obj.id == r?.divisionKey;
          })
            ? "-"
            : divisionDropDown.find((obj) => {
          ...
          subDivision: !subDivisionDropDown?.find((obj) => {
            return obj.id == r?.subDivisionKey;
          })
            ? "-"
            : subDivisionDropDown.find((obj) => {
                return obj.id == r?.subDivisionKey;
              }).subDivision,
          subDivisionMr: !subDivisionDropDown?.find((obj) => {
            return obj.id == r?.subDivisionKey;
          })
            ? "-"
            : subDivisionDropDown.find((obj) => {
          ...
          meterStatus: !meterStatusDropDown?.find((obj) => {
            return obj.id == r?.meterStatusKey;
          })
            ? "-"
            : meterStatusDropDown.find((obj) => {
                return obj.id == r?.meterStatusKey;
              }).meterStatus,
          meterStatusMr: !meterStatusDropDown?.find((obj) => {
            return obj.id == r?.meterStatusKey;
          })
            ? "-"
            : meterStatusDropDown.find((obj) => {
          ...
          loadEquipementCapacity: !loadEquipementCapacityDropDown?.find(
            (obj) => {
              return obj.id == r?.capacity;
            }
          )
            ? "-"
  ...
  const getDivisionData = () => {
    axios
      ...
      .then((res) => {
        let temp = res.data.mstDivisionDao;
        let _res =
          temp &&
          temp.map((r) => {
            return {
    ...
    if (watch("fromDate") && watch("toDate")) {
      ...
      let apiBodyToSend = {
        ...
        meterStatusKey: watch("meterStatusKey")
          ? watch("meterStatusKey")
          : null,
        billAmount: watch("billAmount") ? watch("billAmount") : null,
        consumedUnit: watch("consumedUnit") ? watch("consumedUnit") : null,
        loadEquipementCapacity: watch("loadEquipementCapacity")
          ? watch("loadEquipementCapacity")
          : null,
        loadEquipementDetails: watch("loadEquipementDetails")
          ? watch("loadEquipementDetails")
          : null,
      };
            ...
            if (res?.data.length > 0) {
              ...
              setEngReportsData(
                res?.data?.map((r, i) => ({
                  ...
                  "Sub-Division": !subDivisionDropDown?.find((obj) => {
                    return obj.id == r?.subDivisionKey;
                  })
                    ? "-"
                    : subDivisionDropDown.find((obj) => {
                  ...
                  "Meter Status": !meterStatusDropDown?.find((obj) => {
                    return obj.id == r?.meterStatusKey;
                  })
                    ? "-"
                    : meterStatusDropDown.find((obj) => {
              ...
              setMrReportsData(
                res?.data?.map((r, i) => ({
                  ...
                  "विभागाचे नाव": !divisionDropDown?.find((obj) => {
                    return obj.id == r?.divisionKey;
                  })
                    ? "-"
                    : divisionDropDown.find((obj) => {
                  ...
                  उपविभाग: !subDivisionDropDown?.find((obj) => {
                    return obj.id == r?.subDivisionKey;
                  })
                    ? "-"
                    : subDivisionDropDown.find((obj) => {
                  ...
                  "मीटर स्थिती": !meterStatusDropDown?.find((obj) => {
                    return obj.id == r?.meterStatusKey;
                  })
                    ? "-"
                    : meterStatusDropDown.find((obj) => {
                  ...
                  स्थिती: manageStatus(r.status, language, statusAll),
                }))
              );
  ...
  function generateCSVFile(data) {
    ...
    ws.B4 = {
      ...
      v:
        language == "en"
          ? `Report Name: Bill Segregation Report`
          : `अहवालाचे नाव: बिल पृथक्करण अहवाल`,
    };
    ws.B5 = {
      t: "s",
      v:
                ...
                >
                  ...
                  >
                    <TextField
                      ...
                      variant="standard"
                      InputLabelProps={{
                        shrink: watch("billAmount") ? true : false,
                      }}
                      {...register("billAmount")}
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <FormHelperText>
                        {errors?.tariffCategoryKey
                          ? errors.tariffCategoryKey.message
                          : null}
                      </FormHelperText>
                    </FormControl>
                  </Grid>
