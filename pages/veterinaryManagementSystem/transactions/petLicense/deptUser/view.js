...
const View = () => {
  ...
  useEffect(() => {
    if (watch('antiRabiesVaccinationStatus') === 'y') {
      ...
      if (petAnimalPhoto !== '' && ownerPhotoId !== '' && addressProof !== '') {
        setEnableState(true)
      } else {
        setEnableState(false)
      }
    ...
    if (!!router.query.id) {
      ...
      axios
        ...
        .then((res) => {
          ...
          reset({
            action:
              res.data.status == 'Reassigned by HOD'
                ? 'Reassigned by Clerk'
                : 'Approved by Clerk',
        ...
        axios
          ...
          .then((res) => {
            if (res.status === 200 || res.status === 201) {
              ...
              router.push(
                `/veterinaryManagementSystem/transactions/petLicense/deptUser`
              )
            }
            setLoader(false)
          })
  ...
  return (
    <>
      ...
      <Paper className={styles.main}>
        ...
        <form onSubmit={handleSubmit(finalSubmit)} style={{ padding: '20px' }}>
          ...
          >
            {['Applied', 'Reassigned by HOD'].includes(
