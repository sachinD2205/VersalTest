...
const Index = () => {
  ...
  const finalSubmit = (data) => {
    ...
    axios
      .get(
        `${URLs.VMS}/trnPetLicence/getAllExpiryReportByFromDateAndToDateAndPetType`,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
          },
      ...
      .then((res) => {
        setTable(
          res.data?.trnPetLicenceList?.length > 0
    ...
    <>
      ...
      <Paper className={styles.main}>
        <div className={styles.title}>
          <FormattedLabel id='dateWiseIssuedLicenseExpiry' />
        </div>
