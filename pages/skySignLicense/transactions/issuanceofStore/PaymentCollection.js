...
const Index = () => {
  ...
  const handleExit = () => {
    ...
    if (router?.query?.citizenView) {
      ...
      router.push("/skySignLicense/transactions/issuanceofStore/scrutiny");
    }
  };
  ...
  const handlePay = () => {
    ...
    let testBody = {
      ...
      divertPageLink: "skySignLicense/transactions/issuanceofStore/pgSuccess",
      ...
      axios
        ...
        .then((res) => {
          if (res.status == 200 || res.status == 201) {
            router.push({
              pathname:
                "/skySignLicense/transactions/issuanceofStore/ServiceChargeRecipt",
              query: {
                id: router?.query?.id,
