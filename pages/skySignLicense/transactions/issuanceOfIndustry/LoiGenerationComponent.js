...
const LoiGenerationComponent = (props) => {
  ...
  const [tempTotalEmployees, setTempTotalEmployees] = useState(0);
  const [tempNoOfFlourMillCount, setTempNoOfFlourMillCount] = useState(0);
  ...
  const getCharges = () => {
    let finalBodyForApi;
    if (tempNoOfFlourMillCount > 0) {
      finalBodyForApi = {
        machineCount: tempNoOfFlourMillCount,
      };
    } else {
      finalBodyForApi = {
        totalEmployees: tempTotalEmployees,
      };
    }

    axios
      .post(
        `${urls.SSLM}/master/MstRateChartOfIndustrialLicense/getRateChartData`,
        finalBodyForApi,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
      ...
      .then((r) => {
        ...
        setValue("minEmployees", r?.data?.minEmployees);
        setValue("maxEmployees", r?.data?.maxEmployees);
    ...
    axios
      ...
      .then((r) => {
        ...
        if (r.status === 200) {
          ...
          setValue(
            "machineCount",
            r?.data?.trnIndustryDetailsDao?.machineCount
          );
          setValue(
            "totalEmployees",
            r?.data?.trnIndustryDetailsDao?.totalEmployees
          );
          setTempNoOfFlourMillCount(
            r?.data?.trnIndustryDetailsDao?.machineCount
          );
          setTempTotalEmployees(r?.data?.trnIndustryDetailsDao?.totalEmployees);
        }
      })
      .catch((err) => {
        setLoading(false);
  ...
  useEffect(() => {
    getCharges();
  }, [tempNoOfFlourMillCount, tempTotalEmployees]);
              ...
              {tempNoOfFlourMillCount > 0 ? (
                <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                  <TextField
                    ...
                    InputLabelProps={{
                      shrink:
                        (watch(`machineCount`) ? true : false) ||
                        (router.query.machineCount ? true : false),
                    }}
                    label="machineCount"
                    variant="standard"
                    type="number"
                ...
                <>
                  <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                    <TextField
                      ...
                      InputLabelProps={{
                        shrink:
                          (watch(`totalEmployees`) ? true : false) ||
                          (router.query.totalEmployees ? true : false),
                      }}
                  ...
                  <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                    <TextField
                      ...
                      InputLabelProps={{
                        shrink:
                          (watch(`minEmployees`) ? true : false) ||
                          (router.query.minEmployees ? true : false),
                      }}
                  ...
                  <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                    <TextField
                      ...
                      InputLabelProps={{
                        shrink:
                          (watch(`maxEmployees`) ? true : false) ||
                          (router.query.maxEmployees ? true : false),
                      }}
