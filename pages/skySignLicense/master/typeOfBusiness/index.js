...
const Index = () => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.SSLM}/master/MstBusinessTypes/save`, body)
            .then((res) => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    // Save - DB
    axios
      .post(`${urls.SSLM}/master/MstBusinessTypes/save`, finalBodyForApi)
      .then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
  ...
  const getUnitTypeDetails = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      ...
      .then((r) => {
        console.log(";r", r);
        let result = r.data?.mstBusinessTypesDao;
