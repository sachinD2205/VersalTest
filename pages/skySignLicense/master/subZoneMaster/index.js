...
import schema from "../../../../containers/schema/skysignschema/subZoneMasterschema";
...
const Index = () => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.SSLM}/mstSubZone/save`, body).then((res) => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    // Save - DB
    console.log("Post -----");
    axios.post(`${urls.SSLM}/mstSubZone/save`, finalBodyForApi).then((res) => {
      console.log("res", res);
      if (res.status == 200) {
        fromData.id
  ...
  // Get Table - Data
  const getSubZoneDetails = (_pageSize = 10, _pageNo = 0) => {
    console.log("_pageSize,_pageNo", _pageSize, _pageNo);
    console.log("getLIC ----");
    axios
      .get(`${urls.SSLM}/mstSubZone/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((r) => {
        console.log(";r", r);
        let result = r.data.mstSubZoneDao;
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* <h2>Sub-Zone Master</h2>? */}
          <h2>
            <FormattedLabel id="subZoneMaster" />
          </h2>
        </Box>
        {isOpenCollapse && (
          <div>
                ...
                >
                  ...
                  <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                    <TextField
                      id="standard-basic"
                      // label="Sub-Zone *"
                      label={<FormattedLabel id="subZoneNameEn" />}
                      variant="standard"
                      {...register("subZoneName")}
                      error={!!errors.subZoneName}
                      helperText={
                        errors?.subZoneName ? errors.subZoneName.message : null
                      }
                    />
                  </Grid>
                  <Grid item xl={4} lg={4} md={4} sm={12} xs={12}>
                    <TextField
                      id="standard-basic"
                      // label="Sub-Zone Marathi*"
                      label={<FormattedLabel id="subZoneNameMr" />}
                      variant="standard"
                      {...register("subZoneNameMr")}
                      error={!!errors.subZoneNameMr}
                      helperText={
                        errors?.subZoneNameMr
                          ? errors.subZoneNameMr.message
                          : null
                      }
                    />
                  </Grid>
        ...
        <DataGrid
          ...
          columns={columns}
          onPageChange={(_data) => {
            getSubZoneDetails(data.pageSize, _data);
          }}
          onPageSizeChange={(_data) => {
            console.log("222", _data);
            // updateData("page", 1);
            getSubZoneDetails(_data, data.page);
          }}
        />
      </Paper>
      {/* </BasicLayout> */}
