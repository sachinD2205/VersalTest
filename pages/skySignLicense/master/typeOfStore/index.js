...
const Index = () => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.SSLM}/mstStoreTypes/save`, body)
            .then((res) => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    // Save - DB
    axios
      .post(`${urls.SSLM}/mstStoreTypes/save`, finalBodyForApi)
      .then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
  ...
  // Get Table - Data
  const getUnitTypeDetails = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      ...
      .then((r) => {
        console.log(";r", r);
        let result = r.data?.mstStoreTypesDao;
    ...
    <>
      ...
      >
              ...
              <form onSubmit={handleSubmit(onSubmitForm)}>
                ...
                >
                  ...
                  <Grid item
                    ...
                    <TextField
                      id="standard-basic"
                      label="Store Type Mar"
                      variant="standard"
                      {...register("storeTypeMr")}
                      error={!!errors.storeTypeMr}
                      helperText={
                        errors?.storeTypeMr
                          ? errors.storeTypeMr.message
                          : null
                      }
                    />
                  </Grid>
        ...
        <DataGrid
          ...
          columns={columns}
          onPageChange={(_data) => {
            getUnitTypeDetails(data.pageSize, _data);
          }}
          onPageSizeChange={(_data) => {
            console.log("222", _data);
            // updateData("page", 1);
            getUnitTypeDetails(_data, data.page);
          }}
        />
      </Paper>
      {/* </BasicLayout> */}
