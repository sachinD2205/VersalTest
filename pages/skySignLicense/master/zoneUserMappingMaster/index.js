...
import schema from "../../../../containers/schema/skysignschema/zoneUserMappingschema";
...
const Index = () => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.SSLM}/mstSkySignUserMapping/save`, body)
            .then((res) => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    console.log("Post -----");
    axios
      .post(`${urls.SSLM}/mstSkySignUserMapping/save`, finalBodyForApi)
      .then((res) => {
        console.log("res", res);
        if (res.status == 200) {
          fromData.id
  ...
  const getSubZoneDetails = (_pageSize = 10, _pageNo = 0) => {
    ...
    console.log("getLIC ----");
    axios
      .get(`${urls.SSLM}/mstSkySignUserMapping/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((r) => {
        console.log(";r", r);
        let result = r.data.mstSkySignUserMappingDao;
        ...
        let _res = result?.map((r, i) => {
          ...
          return {
            ...
            zoneName: r.zoneId
              ? zoneNames?.find((obj) => obj?.id == r.zoneId)?.zoneName
              : "-",

            zoneNameMr: r.zoneId
              ? zoneNames?.find((obj) => obj?.id == r.zoneId)?.zoneNameMr
              : "-",
          };
        });
        setDataSource([..._res]);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* <h2>Zone-User Mapping Master</h2> */}
          <h2>
            <FormattedLabel id="zoneUserMappingMaster" />
          </h2>
        </Box>
        {isOpenCollapse && (
          <div>
