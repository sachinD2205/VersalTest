...
import ApplicantDetails from "../../../../components/waterTax/components/ApplicantDetails.js";
import ConnectionDetails from "../../../../components/waterTax/components/ConnectionDetails.js";
import OwnerDetails from "../../../../components/waterTax/components/OwnerDetails.js";
import Document from "../../../../components/waterTax/components/Document.js";
...
} from "../../../../components/waterTax/schema/issuanceOfHawkerLicenseSchema.js";
import IssuanceOfStreetVendorCSS from "../../../../components/waterTax/styles/IssuanceOfHawkerLicenseSteeper.module.css";
import Loader from "../../../../containers/Layout/components/Loader/index.js";
import {
  useGetToken,
  useLoggedInUser,
  useUser,
} from "../../../../containers/reuseableComponents/CustomHooks.js";
...
function getSteps(i) {
  return [
    <strong key={1}>{<FormattedLabel id="applicantDetail" key={2} />}</strong>,
    <strong key={2}>{<FormattedLabel id="ownerDetails" key={3} />}</strong>,
    <strong key={3}>{<FormattedLabel id="connectionDetails" key={4} />}</strong>,
    <strong key={4}>{<FormattedLabel id="documentUpload" key={5} />}</strong>,
  ];
}
...
function getStepContent(step, loadderState) {
    switch (step) {
      case 0:
        return <ApplicantDetails key={2} />;

      case 1:
        return <OwnerDetails key={3} />;

      case 2:
        return <ConnectionDetails key={4} />;
      
      case 3:
        return <Document key={5} loadderState={loadderState} />;
...
const LinaerStepper = () => {
  ...
  const getDocuments = (data) => {
    ...
    axios
      .get(
        `${urls.CFCURL}/master/serviceWiseChecklist/getAllByServiceId?serviceId=${143}`
        ,{headers: {
        ...
        if (localStorage.getItem("applicationRevertedToCititizen") == "true") {
          ...
          axios
            ...
            .catch((error) => {
              setValue("loadderState", false);
              callCatchMethod(error, language);
              console.log("issuanceOfHawkerLicenseUpdateApi", error);
            });
        }
      }
    }
  ...
  const getHawkerCertificateData = () => {
    ...
    axios
      ...
      .catch((error) => {
        setValue("loadderState", false);
        callCatchMethod(error, language);
        console.log("issuanceOfHawkerLicenseGetById", error);
      });
  };
  ...
  useEffect(() => {
    setValue("disabledFieldInputState", false);
    console.log("language23", language)
    if (activeStep == "0") {
  ...
  useEffect(() => {
    console.log("steps", activeStep, language,);
    setDataValidation();
    ...
    if (activeStep == "0") {
      ...
      setDataValidation(DocumentsUploadSchema(language));
    }
  }, [
    activeStep,
    language,
  ...
  //Errors
  useEffect(() => {
    console.log("Errors2213", errors);
  }, [errors]);

  useEffect(() => {
    console.log("addressDetails", watch("addresDetail"));
  }, [watch("addresDetail")]);

  useEffect(() => {
    console.log("language34234", language)
  }, [language])
        ...
        {watch("loadderState") ? (
          ...
          >
            ...
            <div className={IssuanceOfStreetVendorCSS.MainHeaderTitleDiv}>
              {<FormattedLabel id="newWaterConnection" />}
            </div>
                      ...
                      return (
                        <Step key={label} {...stepProps}>
                          <StepLabel
                            ...
                            onClick={()=>{
                              console.log("stepper",indexx,":",label);
                              setActiveStep(indexx)
                              // switch (label.key) {
                              //   case "2":
                              //     setActiveStep(1)
                              //     break;
              ...
              <form onSubmit={methods.handleSubmit(handleNext)}>
                ...
                >
                  ...
                  <DialogContent>
                    ...
                    <ConnectionDetails />
                    <OwnerDetails />
                    <Document loadderState={loadderState}/>
                  </DialogContent>
