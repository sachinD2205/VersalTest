...
import { ConnectionSizeSchema} from "../../../../components/waterTax/schema/connectionSize";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(ConnectionSizeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      // fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/connectionsize/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.connectionSize;
          console.log("consumerCategoryGet", response);
          let _res = response.map((r, i) => {
            return {
              id: r?.id,
              srNo: i + 1,
              connectionSizeInch: r?.connectionSizeInch,
              connectionSizeMM: r?.connectionSizeMM,
              // toDate: r?.toDate,
              // fromDate: r?.fromDate,
              // toDate1:
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/connectionsize/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/connectionsize/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
    ...
    {
      field: "connectionSizeInch",
      headerName: <FormattedLabel id="connectionSizeInch" />,
      description: <FormattedLabel id="connectionSizeInch" />,
      width: 240,
      flex: 3,
    ...
    {
      field: "connectionSizeMM",
      headerName: <FormattedLabel id="connectionSizeMM" />,
      description: <FormattedLabel id="connectionSizeMM" />,
      width: 240,
      flex: 3,
          ...
          <Paper className={ItemMasterCSS.Paper} elevation={5}>
            <ThemeProvider theme={theme}>
              <div className={ItemMasterCSS.MainHeader}>
                {<FormattedLabel id="connectionSize" />}
              </div>
              {isOpenCollapse && (
                <Slide
                  direction="down"
                    ...
                    <FormProvider {...methods}>
                      <form onSubmit={handleSubmit(onSubmitForm)}>
                        ...
                        <TextField
                              ...
                              InputLabelProps={{ shrink: watch("connectionSizeInch")!=null && watch("connectionSizeInch")!=""&& watch("connectionSizeInch")!=undefined ? true :false }}
                              id="standard-basic"
                              label={<FormattedLabel id="connectionSizeInch" required/>}
                              variant="standard"
                              {...register("connectionSizeInch")}
                              error={!!errors?.connectionSizeInch}
                              helperText={
                                errors?.connectionSizeInch ? errors?.connectionSizeInch?.message : null
                              }
                            />
                          ...
                          >
                              ...
                              InputLabelProps={{ shrink: watch("connectionSizeMM")!=null && watch("connectionSizeMM")!=""&& watch("connectionSizeMM")!=undefined ? true :false }}
                              id="standard-basic"
                              label={<FormattedLabel id="connectionSizeMM" required/>}
                              variant="standard"
                              {...register("connectionSizeMM")}
                              error={!!errors?.connectionSizeMM}
                              helperText={
                                errors?.connectionSizeMM ? errors?.connectionSizeMM?.message : null
                              }
                            />
