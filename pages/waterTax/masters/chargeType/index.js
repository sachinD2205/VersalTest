...
import { ChargeTypeSchema } from "../../../../components/waterTax/schema/ChargeType";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(ChargeTypeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/chargeType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.chargeType;
          ...
          let _res = response.map((r, i) => {
            return {
              ...
              chargeTypeNameMr: r?.chargeTypeNameMr,
              toDate: r?.toDate,
              fromDate: r?.fromDate,
              toDate1:
              r?.toDate
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/chargeType/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/chargeType/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
    ...
    {
      field: language == "en" ? "chargeTypeName" : "chargeTypeNameMr",
      headerName: <FormattedLabel id="chargeType" />,
      description: <FormattedLabel id="chargeType" />,
      width: 240,
      flex: 3,

    },
    {
      field: language == "en" ? "chargeTypePrefix" : "chargeTypePrefix",
      headerName: <FormattedLabel id="chargeTypePrefix" />,
      description: <FormattedLabel id="chargeTypePrefix" />,
      width: 240,
      flex: 3,
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                           <TextField
                              ...
                              InputLabelProps={{ shrink: watch("chargeTypePrefix")!=null && watch("chargeTypePrefix")!=""&& watch("chargeTypePrefix")!=undefined ? true :false }}
                              id="standard-basic"
                              label={<FormattedLabel id="chargeTypePrefix" required/>}
                              variant="standard"
                              {...register("chargeTypePrefix")}
                              error={!!errors?.chargeTypePrefix}
                              helperText={
                                errors?.chargeTypePrefix ? errors?.chargeTypePrefix?.message : null
                              }
                            />
                          </Grid>
                          <Grid
                          ...
                          >
                              ...
                              width={270}
                              error={!!errors?.chargeTypeNameEn}
                              helperText={
                                errors?.chargeTypeNameEn
                                  ? errors?.chargeTypeNameEn?.message
                                  : null
                              }
                              key={"chargeTypeName"}
                              fieldName={"chargeTypeName"}
                              updateFieldName={"chargeTypeNameMr"}
                              sourceLang={"en-US"}
                              targetLang={"mr-IN"}
                            />
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.chargeTypeNameMr}
                              helperText={
                                errors?.chargeTypeNameMr
                                  ? errors?.chargeTypeNameMr?.message
                                  : null
                              }
                              key={"chargeTypeNameMr"}
                              fieldName={"chargeTypeNameMr"}
                              updateFieldName={"chargeTypeName"}
                              sourceLang={"mr-IN"}
                              targetLang={"en-US"}
                            />
                          </Grid>
                          <TextField
                              ...
                              style={{ marginLeft: "12vh" ,marginBottom:"2vh"}}
