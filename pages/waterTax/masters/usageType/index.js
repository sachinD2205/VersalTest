...
import { UsageTypeSchema } from "../../../../components/waterTax/schema/usageType";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(UsageTypeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/usageType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.usageType;
          console.log("consumerCategoryGet", response);
          let _res = response.map((r, i) => {
            return {
              id: r?.id,
              srNo: i + 1,
              usageTypeName: r?.usageTypeName,
              usageTypeNameMr: r?.usageTypeNameMr,
              toDate: r?.toDate,
              fromDate: r?.fromDate,
              toDate1:
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/usageType/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/usageType/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
...

    {
      ...
      headerName: <FormattedLabel id="usageTypeName" />,
      description: <FormattedLabel id="usageTypeName" />,
      width: 240,
      flex: 3,
          ...
          <Paper className={ItemMasterCSS.Paper} elevation={5}>
            <ThemeProvider theme={theme}>
              <div className={ItemMasterCSS.MainHeader}>
                {<FormattedLabel id="usageTypeName" />}
              </div>
              {isOpenCollapse && (
                <Slide
                  direction="down"
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                            <Translation
                              ...
                              key={"usageTypeName"}
                              fieldName={"usageTypeName"}
                              updateFieldName={"usageTypeNameMr"}
                              sourceLang={"en-US"}
                              targetLang={"mr-IN"}
                            />
                          </Grid>
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.usageTypeNameMr}
                              helperText={
                                errors?.usageTypeNameMr
                                  ? errors?.usageTypeNameMr?.message
                                  : null
                              }
                              key={"usageTypeNameMr"}
                              fieldName={"usageTypeNameMr"}
                              updateFieldName={"usageTypeName"}
                              sourceLang={"mr-IN"}
                              targetLang={"en-US"}
                            />
                          </Grid>
