...
import { RoadTypeSchema } from "../../../../components/waterTax/schema/roadType";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(RoadTypeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/roadType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.roadType;
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/roadType/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/roadType/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
...

    {
      ...
      headerName: <FormattedLabel id="roadTypeName" />,
      description: <FormattedLabel id="roadTypeName" />,
      width: 240,
      flex: 3,
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                            <Translation
                              ...
                              helperText={
                                errors?.roadTypeName
                                  ? errors?.roadTypeName?.message
                                  : null
                              }
                              key={"roadTypeName"}
                              fieldName={"roadTypeName"}
