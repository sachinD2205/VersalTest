...
import { HousingTypeSchema } from "../../../../components/waterTax/schema/housingType";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(HousingTypeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/housingType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.housingType;
          console.log("consumerCategoryGet", response);
          let _res = response.map((r, i) => {
            return {
              id: r?.id,
              srNo: i + 1,
              housingTypeName: r?.housingTypeName,
              housingTypeNameMr: r?.housingTypeNameMr,
              toDate: r?.toDate,
              fromDate: r?.fromDate,
              toDate1:
              r?.toDate
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/housingType/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/housingType/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
    ...
    {
      field: language == "en" ? "housingTypeName" : "housingTypeNameMr",
      headerName: <FormattedLabel id="housingTypeName" />,
      description: <FormattedLabel id="housingTypeName" />,
      width: 240,
      flex: 3,
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.housingTypeName}
                              helperText={
                                errors?.housingTypeName
                                  ? errors?.housingTypeName?.message
                                  : null
                              }
                              key={"housingType"}
                              fieldName={"housingTypeName"}
                              updateFieldName={"housingTypeNameMr"}
                              sourceLang={"en-US"}
                              targetLang={"mr-IN"}
                            />
                          </Grid>
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.housingTypeNameMr}
                              helperText={
                                errors?.housingTypeNameMr
                                  ? errors?.housingTypeNameMr?.message
                                  : null
                              }
                              key={"housingTypeNameMr"}
                              fieldName={"housingTypeNameMr"}
                              updateFieldName={"housingTypeName"}
                              sourceLang={"mr-IN"}
                              targetLang={"en-US"}
                            />
                          </Grid>
