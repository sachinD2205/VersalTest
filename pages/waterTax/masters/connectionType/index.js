...
import { ConnectionTypeSchema } from "../../../../components/waterTax/schema/connectionType";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(ConnectionTypeSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/connectionType/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.connectionType;
          ...
          let _res = response.map((r, i) => {
            return {
              ...
              connectionTypeNameMr: r?.connectionTypeNameMr,
              toDate: r?.toDate,
              fromDate: r?.fromDate,
              toDate1:
              r?.toDate
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/connectionType/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/connectionType/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
    ...
    {
      field: language == "en" ? "connectionType" : "connectionTypeNameMr",
      headerName: <FormattedLabel id="connectionType" />,
      description: <FormattedLabel id="connectionType" />,
      width: 240,
      flex: 3,
          ...
          <Paper className={ItemMasterCSS.Paper} elevation={5}>
            <ThemeProvider theme={theme}>
              <div className={ItemMasterCSS.MainHeader}>
                {<FormattedLabel id="connectionType" />}
              </div>
              {isOpenCollapse && (
                <Slide
                  direction="down"
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                            <Translation
                              ...
                              updateFieldName={"connectionTypeNameMr"}
                              sourceLang={"en-US"}
                              targetLang={"mr-IN"}
                            />
                          </Grid>
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.connectionTypeNameMr}
                              helperText={
                                errors?.connectionTypeNameMr
                                  ? errors?.connectionTypeNameMr?.message
                                  : null
                              }
                              key={"connectionTypeNameMr"}
                              fieldName={"connectionTypeNameMr"}
                              updateFieldName={"connectionType"}
                              sourceLang={"mr-IN"}
                              targetLang={"en-US"}
                            />
                          </Grid>
