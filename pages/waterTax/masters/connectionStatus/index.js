...
import { ConnectionStatusSchema } from "../../../../components/waterTax/schema/connectionStatus";
...
const Index = () => {
  ...
  const methods = useForm({
    criteriaMode: "all",
    resolver: yupResolver(ConnectionStatusSchema(language)),
    mode: "onChange",
    defaultValues: {
      id: null,
      fromDate: null,
  ...
  const getConsumerCategory = (_pageSize = 10, _pageNo = 0) => {
    ...
    axios
      .get(`${urls.WTURL}/master/connectionStatus/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
        },
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          ...
          let response = res?.data?.connectionStatus;
          ...
          let _res = response.map((r, i) => {
            return {
              ...
              connectionStatusMr: r?.connectionStatusMr,
              toDate: r?.toDate,
              fromDate: r?.fromDate,
              toDate1:
              r?.toDate
  ...
  const onSubmitForm = (fromData) => {
    ...
    axios
      .post(`${urls.WTURL}/master/connectionStatus/save`, finalBodyForApi)
      .then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios
            .post(`${urls.WTURL}/master/connectionStatus/save`, body)
            .then((res) => {
              if (res.status == 200) {
                swal({
                  title:
  ...
  const columns = [
    ...
    {
      field: language == "en" ? "connectionStatus" : "connectionStatusMr",
      headerName: <FormattedLabel id="connectionStatus" />,
      description: <FormattedLabel id="connectionStatus" />,
      width: 240,
      flex: 3,
          ...
          <Paper className={ItemMasterCSS.Paper} elevation={5}>
            <ThemeProvider theme={theme}>
              <div className={ItemMasterCSS.MainHeader}>
                {<FormattedLabel id="connectionStatus" />}
              </div>
              {isOpenCollapse && (
                <Slide
                  direction="down"
                        ...
                        <Grid container style={{ marginBottom: "7vh" }}>
                          ...
                          >
                            <Translation
                              ...
                              updateFieldName={"connectionStatusMr"}
                              sourceLang={"en-US"}
                              targetLang={"mr-IN"}
                            />
                          </Grid>
                          ...
                          >
                            <Translation
                              ...
                              width={270}
                              error={!!errors?.connectionStatusMr}
                              helperText={
                                errors?.connectionStatusMr
                                  ? errors?.connectionStatusMr?.message
                                  : null
                              }
                              key={"connectionStatusMr"}
                              fieldName={"connectionStatusMr"}
                              updateFieldName={"connectionStatus"}
                              sourceLang={"mr-IN"}
                              targetLang={"en-US"}
                            />
                          </Grid>
