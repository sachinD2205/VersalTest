...
import schema from "../../../../containers/schema/newsRotationManagementSystem/masters/dhoran";
...
const Index = () => {
  ...
  const onSubmitForm = (fromData) => {
    ...
    // if (btnSaveText === "Save") {
    axios.post(`${urls.NRMS}/budget/save`, _body)
    .then((res) => {
      if (res.status == 201) {
        fromData.id
          ? sweetAlert(
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      ...
      swal({
        ...
        console.log("inn", willDelete);
        if (willDelete === true) {
          axios.post(`${urls.NRMS}/budget/save`, body)
          .then((res) => {
            console.log("delet res", res);
            if (res.status == 201) {
              swal(
  ...
  const columns = [
    ...
    {
      field: language == "en" ? "financialYearName" : "financialYearNameMr",
