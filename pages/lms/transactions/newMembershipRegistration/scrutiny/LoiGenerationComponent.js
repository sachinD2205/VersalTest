...
const LoiGenerationComponent = (props) => {
    ...
    if (getValues("serviceId")) {
      ...
      axios
        ...
        .get(
          `${
            urls.LMSURL
          }/libraryRateCharge/getByServiceId?serviceId=${getValues(
            "serviceId"
          )}`,
          {
            headers: {
      ...
      axios
        ...
        .then((r) => {
          ...
          if (r.status === 200) {
            ...
            setLibraryType(
              libraryList.find((library) => library.id == r.data.libraryKey)
                ?.libraryType
            );
            console.log(
              "hotay change",
  ...
  useEffect(() => {
    // let total = 0; //replaced as monthlyRs
    let monthlyRs = 0;
    let deposit = 0;
    let fees = 0;
    ...
    if (tempMonths == 12) {
      ...
      if (libraryType == "L") {
        ...
        setValue("loi.amount", Number(monthlyRs * tempMonths + deposit + fees));
        // setValue("loi.totalInWords", toWords.convert(Number(200 + deposit)));
        setValue(
          "loi.totalInWords",
          toWords.convert(Number(monthlyRs * tempMonths + deposit + fees))
        );
      } else {
        ...
        // setValue(
        //   "loi.totalInWords",
        //   toWords.convert(Number(total * tempMonths + deposit))
        // );
        setValue(
          "loi.totalInWords",
          toWords.convert(Number(monthlyRs * tempMonths + deposit + fees))
  ...
  useEffect(() => {
    console.log("loi.amount", watch("loi.amount"));
  }, [watch("loi.amount")]);
          ...
          <form onSubmit={handleSubmit(handleNext)}>
            ...
            <div>
              ...
              >
                ...
                {` *Service Charge Type Name conventions 1) C :- Charge 2) D :-
              Deposit 3) F :- Fees`}
              </Typography>
            </div>
              ...
              >
                <Stack spacing={5} direction="row">
                  <Button
                    ...
                    type="submit"
                    variant="contained"
                    disabled={watch("loi.amount") > "0" ? false : true}
                    // onClick={() =>
                    //   router.push({
                    //     pathname:
