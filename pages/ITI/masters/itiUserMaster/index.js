...
import itiUserSchema from "../../../../containers/schema/iti/masters/itiUserSchema";
...
const Index = () => {
  const {
    ...
    criteriaMode: "all",
    resolver: yupResolver(itiUserSchema),
    mode: "onChange",
  });
  ...
  const [allItiUserList, setAllItiUserList] = useState([]);
  ...
  const getItiAllUser = () => {
    axios
      .get(`${urls.CFCURL}/master/user/getUserByApplication?applicationId=32`, {
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      })
  ...
  useEffect(() => {
    getItiUserMaster();
  }, [fetchData, itiKeys, allItiUserList]);

  // Get Table - Data
  const getItiUserMaster = (
    _pageSize = 10,
    _pageNo = 0,
    _sortBy = "id",
    _sortDir = "desc"
    ...
    setLoading(true);
    axios
      .get(`${urls.SCHOOL}/mstUserItIMapping/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
          sortBy: _sortBy,
      ...
      .then((r) => {
        console.log("r", r);
        let result = r?.data?.mstUserItIMappingDao;
        ...
        let _res = result?.map((r, i) => {
          ...
          return {
            ...
            userId: r?.userId,
            usersNameEn: allItiUserList?.find((obj) => obj?.id == r?.userId)
              ?.usersNameEn,
            usersNameMr: allItiUserList?.find((obj) => obj?.id == r?.userId)
              ?.usersNameMr,
          };
        });
        setData({
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      ...
      const tempData = axios
        .post(`${urls.SCHOOL}/mstUserItIMapping/save`, _body, {
          headers: {
            Authorization: `Bearer ${userToken}`,
          },
        })
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          setLoading(true);
          axios
            .post(`${urls.SCHOOL}/mstUserItIMapping/save`, body, {
              headers: {
                Authorization: `Bearer ${userToken}`,
              },
            })
                        ...
                        <FormControl sx={{ width: 230 }}>
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              ...
                              >
                                {itiKeys &&
                                  itiKeys?.map((iti, index) => (
                                    <MenuItem key={index} value={iti?.id}>
                                      {iti?.itiName}
                                    </MenuItem>
                                  ))}
                              </Select>
          ...
          <>
            {showTable && (
              <DataGrid
                ...
                columns={columns}
                onPageChange={(_data) => {
                  getItiUserMaster(data.pageSize, _data);
                }}
                onPageSizeChange={(_data) => {
                  console.log("222", _data);
                  // updateData("page", 1);
                  getItiUserMaster(_data, data.page);
                }}
              />
            )}
          </>
