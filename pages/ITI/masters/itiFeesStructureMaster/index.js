...
import itiFeesStructureMasterSchema from "../../../../containers/schema/iti/masters/itiFeesStructureMasterSchema";
...
const Index = () => {
  const {
    ...
    criteriaMode: "all",
    resolver: yupResolver(itiFeesStructureMasterSchema),
    mode: "onChange",
  });
  ...
  useEffect(() => {
    getItiFeesStructureMaster();
  }, [fetchData]);

  // Get Table - Data
  const getItiFeesStructureMaster = (
    _pageSize = 10,
    _pageNo = 0,
    _sortBy = "id",
    _sortDir = "desc"
    ...
    axios
      ...
      .then((r) => {
        console.log("r", r);
        let result = r?.data?.mstItIFeesDao;
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      ...
      const tempData = axios
        .post(`${urls.SCHOOL}/mstItIFees/save`, _body, {
          headers: {
            Authorization: `Bearer ${userToken}`,
          },
        })
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          setLoading(true);
          axios
            .post(`${urls.SCHOOL}/mstItIFees/save`, body, {
              headers: {
                Authorization: `Bearer ${userToken}`,
              },
            })
  ...
  const columns = [
    ...
    {
      field: "locatedAt",
      headerName: labels.locationName,
      flex: 1,
      align: "center",
      headerAlign: "center",
    },
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h2>{labels.itiFeesStructureMaster}</h2>
        </Box>
        <Box
          sx={{
            marginLeft: 10,
                    ...
                    <Grid container>
                      ...
                      >
                        <TextField
                          ...
                          InputLabelProps={{
                            style: { fontSize: 15 },
                            shrink: watch("locatedAt") ? true : false,
                          }}
                          helperText={
                      ...
                      >
                        <TextField
                          ...
                          InputLabelProps={{
                            style: { fontSize: 15 },
                            shrink: watch("fees") ? true : false,
                          }}
                          helperText={errors?.fees ? labels.feesReq : null}
                        />
                      </Grid>
                      {/* deposit / cautionFees */}
                      <Grid
            ...
            <>
              {showTable && (
                <DataGrid
                  ...
                  columns={columns}
                  onPageChange={(_data) => {
                    getItiFeesStructureMaster(data.pageSize, _data);
                  }}
                  onPageSizeChange={(_data) => {
                    console.log("222", _data);
                    // updateData("page", 1);
                    getItiFeesStructureMaster(_data, data.page);
                  }}
                />
              )}
            </>
