...
const Index = () => {
  ...
  // useEffect(() => {
  //   if (router?.query) {
  //     if (router?.query?.pageMode == "LoiCollection") {
  //       setApplicationData(router?.query?.data);
  //       // reset(router?.query);
  //       // reset(record.row);
  //       setValue("serviceName", router?.query?.serviceId);
  //       loiCollectionOpen();
  //       // setLoiCollection(false)
  //     }
  //   }
  // }, []);
  ...
  const getProvisionalNoc = (
    ...
    axios
      .get(`${urls.FbsURL}/transaction/iodNoc/getAll`, {
        params: {
          pageSize: _pageSize,
          pageNo: _pageNo,
          sortBy: _sortBy,
