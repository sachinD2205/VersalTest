...
const LoiCollectionComponent = () => {
  ...
  const [loaderStateDisabled, setLoaderStateDisabled] = useState(false);
  ...
  const handlePay = () => {
    ...
    if (watch("modeOfPayment") == 3) {
      ...
      let testBody = {
        ...
        language: "EN",
        moduleId: "FBS",
        amount: total?.totalNocAmount,
        // amount: 2000,
        divertPageLink:
          "FireBrigadeSystem/transactions/provisionalBuildingNoc/pgSuccess",
        loiId: getValues("loi.id"),
        loiNo: getValues("loi.loiNO"),
        ccAvenueKitLtp: ccAvenueKitLtp,
        serviceId: 76,
        applicationId: Number(router?.query?.id),
        applicationNo: String(router?.query?.applicationNumber),
      ...
      axios
        ...
        .then((res) => {
          if (res.status == 200 || res.status == 201) {
            ...
            localStorage.setItem("selectedServiceId", 76);
  ...
  useEffect(() => {
    console.log("1234", router?.query?.applicationNumber);
    setLoaderStateDisabled(true);
    if (router?.query?.id) {
      axios
        .get(
        ...
        .then((res) => {
          setLoaderStateDisabled(false);

          console.log("221297", res?.data);
          if (res.status == 200) {
            // const applicantData = res?.data?.applicantDTLDao.map((a) => {
            //   console.log("aaaaaaa", a);
            // });
            // reset(res?.data?.applicantDTLDao);
            setValue(
              "applicantDTLDao.applicantName",
              res?.data?.applicantDTLDao?.applicantName
            );
            setValue(
              "applicantDTLDao.applicantMiddleName",
              res?.data?.applicantDTLDao?.applicantMiddleName
            );
            setValue(
              "applicantDTLDao.applicantLastName",
              res?.data?.applicantDTLDao?.applicantLastName
            );
            setValue(
              "applicantDTLDao.applicantNameMr",
              res?.data?.applicantDTLDao?.applicantNameMr
            );
            setValue(
              "applicantDTLDao.applicantMiddleNameMr",
              res?.data?.applicantDTLDao?.applicantMiddleNameMr
            );
            setValue(
              "applicantDTLDao.applicantLastNameMr",
              res?.data?.applicantDTLDao?.applicantLastNameMr
            );
            setValue(
              "applicantDTLDao.applicantName",
              res?.data?.applicantDTLDao?.applicantName
            ...
            setValue(
              "applicantDTLDao.applicantEmailId",
              res?.data?.applicantDTLDao?.applicantEmailId
            );

            setValue(
              "applicantDTLDao.applicantMobileNo",
              res?.data?.applicantDTLDao?.applicantMobileNo
            );

            // Set LOI Details
            setValue("loi.loiNO", res?.data?.loi?.loiNO);

            setValue("loi.id", res?.data?.loi?.id);

            setValue("loi.loiDate", res?.data?.loi?.loiDate);
  ...
  //     .then((res) => {
  //       console.log("11", res);
  //       setApplicableCharages(res?.data?.map((charge, i) => {
  //         return {
  //           ...charge,
  //           srNo: i + 1,
  //         }
  ...
  //         totalEditableGrossNocAmount = totalEditableGrossNocAmount + charge.finalGrossNocAmount;
  //       });
  ...
  const handleNext = (data) => {
    const finalBodyForApi = {
      // ...data,
      role: "LOI_COLLECTION",
      NocId: Number(router?.query?.id),

      loi: {
        // buildingNocApplicableCharagesDao: [...applicableCharages],
        // provisionalNocId: router?.query?.id,
      },
