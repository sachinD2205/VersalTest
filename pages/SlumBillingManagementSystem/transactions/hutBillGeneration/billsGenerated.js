...
const Index = () => {
  ...
  const getAllForBilling = (year) => {
    ...
    if (year) {
      axios
        .get(`${urls.SLUMURL}/trnBill/getBillsByYear?year=${year}`, {
          headers: headers,
        })
        .then((res) => {
          setTable(
  ...
  const generateBill = () => {
    ...
    axios
      .post(`${urls.SLUMURL}/trnBill/bulk/generate`, bodyForApi, config)
      .then((res) => {}).catch((err)=>{
        cfcErrorCatchMethod(err, false);
      });
  };
        ...
        return (
          <Button
            ...
            onClick={() => {
              router.push({
                pathname:
                  "/SlumBillingManagementSystem/transactions/hutBillGeneration/bill",
