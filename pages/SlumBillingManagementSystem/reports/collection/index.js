...
const Index = () => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: language === "en" ? "Collection" : "संकलन",
  });
  ...
  const onSubmitFunc = () => {
    if (watch("fromDate") && watch("toDate")) {
      ...
      setLoading(true);
      axios
        .post(`${urls.SLUMURL}/report/getCollectionReport`, apiBodyToSend, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
  ...
  function generateCSVFile(data) {
    ...
    const fileName = language == "en" ? "Collection" : "संकलन";
    let col = () => {};
    col();
    const fileType =
   ...
   const columnWidths = keyNames.map((column) => column.length);
    ...
    ws.F4 = {
      t: "s",
      v: language == "en" ? `Report Name: Collection` : `अहवालाचे नाव: संकलन`,
      s: dataStyle,
    };
    ws.F5 = {
      t: "s",
