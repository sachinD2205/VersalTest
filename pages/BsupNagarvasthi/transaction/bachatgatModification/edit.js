...
const BachatGatCategory = () => {
  ...
  //       `${
  //         urls.CFCURL
  //       }/master/zoneWardAreaMapping/getAreaName?moduleId=23&areaName=${watch(
  //         "areaName"
  //       )}`
  //     )
  //     .then((res) => {
  ...
  const setDataOnForm = () => {
    ...
    bankDoc && setDocumentUpdate([...bankDoc]);
  };
      ...
      .then((res) => {
        ...
        if (res.status == 201) {
          sweetAlert({
            text:
              language === "en"
                ? ` Bachatgat modification is updated Successfully. Your application no. is : ${
                    res.data.message.split("[")[1].split("]")[0]
                  } `
                : `बचतगट सुधारणा यशस्वीरित्या अद्यतनित केली आहे. तुमचा अर्ज क्र. आहे: ${
                    res.data.message.split("[")[1].split("]")[0]
                  } `,
            icon: "success",
            button: language === "en" ? "Ok" : "ठीक आहे",
          ...
          <form onSubmit={handleSubmit(onSubmitForm)}>
            <Grid container spacing={2} style={{ padding: "1rem" }}>
              ...
              <Grid item xs={12} sm={12} md={6} lg={3} xl={3}>
                ...
                >
                  <InputLabel id="demo-simple-select-standard-label">
                    <FormattedLabel id="zoneNames" required />
                  </InputLabel>
                  <Controller
                    render={({ field }) => (
                      <Select
