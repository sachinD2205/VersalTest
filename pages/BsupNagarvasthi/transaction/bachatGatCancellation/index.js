...
const BachatGatCategoryCancellation = () => {
  ...
  const getBachatgatCategoryCancellationTrn = (_pageSize = 10, _pageNo = 0) => {
    {
      setIsLoading(true);
      axios
        .get(`${urls.BSUPURL}/trnBachatgatCancellation/getAll`, {
          headers: headers,
          params: {
            pageSize: _pageSize,
            pageNo: _pageNo,
  ...
  useEffect(() => {
    if (regDetails != null) setDataToTable();
  }, [regDetails, language]);

  // set data to table
  const setDataToTable = () => {
    let data = regDetails;
    let result = data.trnBachatgatCancellationList;
      ...
      if (wardNames && zoneNames && crAreaNames) {
        let _res = result?.map((r, i) => {
          return {
            ...
            statusVal: r.status,
            cancelDate: moment(r.cancelDate).format("DD/MM/YYYY HH:mm"),
            bachatgatName: r.bachatgatName,
            bachatgatRegistrationDate: r.bachatgatRegistrationDate,
  ...
  const columns = [
    ...
    {
      field: "bachatgatRegistrationDate",
  ...
  const setRemark = (isApprovedCheck) => {
    let remarkData = [];
    for (var i = 0; i < regDetails.trnBachatgatCancellationList.length; i++) {
      if (
        regDetails.trnBachatgatCancellationList.find(
          (obj) => obj.id === selectedRows[i]
        )
      ) {
        let abc = regDetails.trnBachatgatCancellationList.find(
          (obj) => obj.id === selectedRows[i]
        );
        const abcd = { ...abc };
        (abcd.isApproved =
    ...
    const tempData = axios
      .post(`${urls.BSUPURL}/trnBachatgatCancellation/save`, remarkData, {
        headers: {
          headers: headers,
        },
      })
