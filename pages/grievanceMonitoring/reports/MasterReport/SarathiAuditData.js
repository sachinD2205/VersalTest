...
import SarathiAuditDataCSS from "./SarathiAuditData.module.css";
...
const SarathiAuditData = () => {
  ...
  const [engSarathiAuditedTableData, setEngSarathiAuditedTableData] = useState([]);
  const [mrSarathiAuditedTableData, setMrSarathiAuditedTableData] = useState([]);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle:language==='en'?watch('ClickFiledName'):watch('ClickFiledNameMr')
  });

  // SarathiAuditDataDayTableColumns
  const SarathiAuditDataDayTableColumns = [
    {
      field: "srNo",
      ...
      renderCell: (record) => {
        ...
        return (
          <>
            <IconButton
              onClick={() => {
                console.log("bhaispadalakatokennumber", record?.row?.tokenNo);
                handleRowClickSarathiAuditDayWiseDetails(record?.row?.tokenNo);
              }}
            >
              {language==='en'?<Button variant="contained" size="small">
             More Details
...
console.log('fileName ',fileName);
  ...
  // handleRowClickSarathiAuditDayWiseDetails
  const handleRowClickSarathiAuditDayWiseDetails = (tokenNo) => {
      ...
      .then((res) => {
        if (res?.status == 200 || res?.status == 201) {
          if (
            ...
            sweetalert(language==='en'?"Not Found !":"सापडले नाही!", language==='en'?"Record Not Found!":"रेकॉर्ड सापडला नाही!", "warning");
            setValue("loadderState", false);
          }
        } else {
          setValue("loadderState", false);
  ...
  useEffect(() => {
    let SarathiAuditData = watch("SarathiAuditData");
    if (
      SarathiAuditData != null &&
      SarathiAuditData.length != "0" &&
      SarathiAuditData != undefined
    ) {
      // added srNo in array objects
      let withSrNoSarathiAuditData = SarathiAuditData?.map((data, index) => {
        return {
          srNo: index + 1,
          ...data,
      ...
      setSarathiAuditedTableData(withSrNoSarathiAuditData);

      const _enAuditD = SarathiAuditData.map((item) => ({
      ...
      const _mrAuditD = SarathiAuditData.map((item) => ({
      ...
      setEngSarathiAuditedTableData(_enAuditD);
      setMrSarathiAuditedTableData(_mrAuditD);
    }
  }, [watch("SarathiAuditData")]);
    ...
    <>
      ...
      >
        ...
        <Button
          ...
          onClick={() =>
            language == "en"
              ? generateCSVFile(engSarathiAuditedTableData)
              : generateCSVFile(mrSarathiAuditedTableData)
          }
        >
      ...
      <DataGrid
        ...
        columns={SarathiAuditDataDayTableColumns}
        pageSize={pageSize}
...
class ComponentToPrintNew extends React.Component {
  render() {
    ...
    console.log("SarathiAuditedTableData", SarathiAuditedTableData);
    ...
    return (
      <>
        <table className={SarathiAuditDataCSS.table}>
          <tbody>
            <tr>
              <td className={SarathiAuditDataCSS.Table1Header} colSpan={9}>
                {language == "en" ? ReportNameEnglish : ReportNameMarathi}
              </td>
            </tr>
            <tr>
              <td
                className={`${SarathiAuditDataCSS.TableTd} ${SarathiAuditDataCSS.TableTh}`}
                colSpan={1}
              >
            ...
            {SarathiAuditedTableData &&
              SarathiAuditedTableData.map((data, index) => (
                <tr>
                  <td className={SarathiAuditDataCSS.TableTd} colSpan={1}>
...
export default SarathiAuditData;
