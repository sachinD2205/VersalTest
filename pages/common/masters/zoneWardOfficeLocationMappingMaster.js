...
const Index = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const bodyForAPI = {
      ...
      ward: Number(data.wardId),
      department: "",
      officeLocation: Number(data.officeLocation),
    };

    let body = data?.ward?.map((areaObj, index) => {
      return {
        zone: Number(data.zone),
    ...
    await axios
      .post(
        `${urls.CFCURL}/master/zoneWardOfficeLoactionMapping/saveAll`,
        body,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <>
            {params.row.activeFlag == "Y" ? (
              <IconButton
                ...
                onClick={() => {
                  getWardsByZoneId(params?.row?.zone);
                  setBtnSaveText("Update"),
                    setID(params.row.id),
                    setIsOpenCollapse(true),
                    setSlideChecked(true);
                  setButtonInputState(true);
                  console.log("params.row: ", params.row, _wardList);

                  reset(params.row);
                  setValue("ward", [
                    _wardList.find((area) => area.wardId === params.row.ward),
                  ]);
                              ...
                              render={({ field }) => (
                                <Autocomplete
                                  ...
                                  isOptionEqualToValue={(option, value) => {
                                    return option.wardId === value.wardId;
                                  }}
                                  renderOption={(
                                    props,
                                    option,
