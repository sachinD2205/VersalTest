...
const Index = () => {
  ...
  const handlePay = () => {
    ...
    console.log("getValuesloi", dataa?.loi?.id);
        ...
        axios
          ...
          .then((res) => {
            if (res.status == 200 || res.status == 201) {
              ...
              localStorage.setItem("selectedServiceId", 12);
  ...
  useEffect(() => {
    let chargeee = null;
    if (router.query.serviceId == 12) {
      chargeee =
        Number(dataa?.marriageServiceCharges) + Number(dataa?.penaltyCharge);
      console.log("nmr");
    } else if (router.query.serviceId == 14) {
      console.log("rmbc");
      chargeee = Number(dataa?.marriageServiceCharges);
    } else {
      console.log("nono");
    }
    console.log("serviceID", router.query.serviceId);
        ...
        >
          ...
          <div className={styles.appDetails}>
            ...
            <table className={styles.report_table}>
              ...
              <tbody>
                {dataa?.serviceCharges.map((row, i) => (
                  <tr key={row.id}>
                    <td colSpan={4}>{i + 1}</td>
                    <td colSpan={4}>
                      {row.serviceChargesId == 8
                        ? row.chargeNameMr +
                          "(" +
                          row.rate +
                          ")" +
                          " X " +
                          dataa?.loi?.noOfCopies +
                          " (प्रति)"
                        : row.chargeNameMr}
                    </td>
                    <td colSpan={4}>
                      {row.serviceChargesId == 8
                        ? row.rate * dataa?.loi?.noOfCopies
                        : row.rate}
                    </td>
                  </tr>
                ))}
