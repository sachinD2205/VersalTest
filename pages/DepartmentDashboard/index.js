...
const DepartmentDashboard = (props) => {
  ...
  const sso = localStorage.getItem("isSsoDashboard");
  ...
  const handleLogout = () => {
    console.log("sso", sso);
    if (sso) {
      console.log("sso__", sso);
      logoutIdam();
      // window.open('http://localhost:8090/cfc/api/emas/logoutIdam','_self')
    } else {
      setAnchorEl(null);
  ...
  const logoutIdam = () => {
    axios
      ...
      // .get("http:///192.168.1.105:8090/cfc/api/emas/logoutIdam")
