...
const Vakalatnama = () => {
    ...
    axios
      ...
      .then((res) => {
        if (res.status == 200 || res.status == 201) {
          sweetAlert(
            // "Vakalatnama Approved",
            language === "en" ? "Vakalatnama Approved" : "वकलतनामा मंजूर",
            // "vakalatnama is approved successfully!",
            language === "en"
              ? "vakalatnama is approved successfully!"
              : "वकलतनामा यशस्वीरित्या मंजूर झाला आहे!",
            "success"
          );
  ...
  const getVakalatnama = () => {
    axios
      .get(`${urls.LCMSURL}/vakalatnama/getAll`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((res) => {
        setVakalatnama(
          res.data.trnVakalatnamaList.map((r, i) => ({
            id: r.id,
            // courtName: r.courtName,
            // courtMr: r.courtMr,
          }))
        );
        setVakalatanamaId(res.data.trnVakalatnamaList[0].id);
      })
      ?.catch((err) => {
        console.log("err", err);
        callCatchMethod(err, language);
    ...
    <>
      <form onSubmit={handleSubmit(onSubmitForm)}>
        ...
        >
          ...
          <Grid item>
            <Button variant="contained" type="submit" endIcon={<DoneAll />}>
              {/* Approve */}
