...
const LoiGenerationComponent = (props) => {
  ...
  const getServiceCharges = () => {
    axios
      .get(
        `${urls.CFCURL}/master/servicecharges/getByServiceId?serviceId=${35}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const getSwimmingBookingData = () => {
    axios
      .get(`${urls.SPURL}/swimmingPool/getById?id=${props?.data?.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    axios
      ...
      .then((res) => {
        if (res.status == 200) {
          ...
          router.push({
            pathname:
              "/sportsPortal/transaction/swimmingPool/scrutiny/LoiGenerationReciptmarathi",
            query: {
