...
const Index = () => {
  ...
  // //  Api Calls ===================================>
  // approve or revert button disabled state based on docs action
  ...
  const getSwimmingData = () => {
    setLoadderState(true);
    console.log("hjbhjb", loadderState);
    axios
      .get(`${urls.SPURL}/swimmingBookingMonthly/getAll`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((resp) => {
        // const applicantNameMr =
        //   resp?.data.firstNameMr +
        //   resp?.data.firstNameMr +
        //   resp?.data.firstNameMr;
        ...
        if (resp.status == 200) {
          ...
          console.log("Loader State", resp?.data.swimmingBooking);
          ...
          // const data = {
          //   ...resp?.data.swimmingBooking,
          //   applicantNameMr: applicantNameMr,
          // };

          const data = resp?.data.swimmingBooking?.map((data, index) => {
            return {
              ...data,
              srNo: index + 1,
              applicantNameMr:
          ...
          //   }),
          // )
          console.log("dsfdssj", data);
          console.log("tyfygyjhbg53", data.applicantNameMr);
  ...
  useEffect(() => {
    console.log("_______________JJJJJJJJJJ", watch("attachmentList"));
    if (watch("attachmentList")?.length > 0) {
      let _a = watch("attachmentList")?.find(
        (obj) => obj?.docStatus === null || obj?.docStatus === "Reject"
      )
        ? false
        : true;
      // setAllDocsApproved(_a);
    }
                      ...
                      <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                        ...
                        >
                          ...
                          <Button
                            variant="contained"
                            // disabled={
                            //   watch("docsRevert") === "YES" ? false : true
                            // }
                            onClick={() => {
                              remarkFun("Revert");
                            }}
