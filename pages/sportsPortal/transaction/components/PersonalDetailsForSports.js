...
const PersonalDetailsForSports = ({ readOnly = false }) => {
  ...
  const addressChange = (e) => {
    if (e?.target?.checked == true) {
      ...
      setValue("adddressMr", watch("addressMr"));
  ...
  const filtredGender = () => {
    if (watch("title") == 1 || watch("title") == 6) {
      const x = [genders?.find((o) => o?.id == 1)];
      setValue("gender", x[0]?.id);
    } else if (watch("title") == 2 || watch("title") == 3) {
      const x = [genders?.find((o) => o?.id == 2)];
      setValue("gender", x[0]?.id);
    } else if (watch("title") == 18) {
      const x = [genders?.find((o) => o?.id == 3)];
      setValue("gender", x[0]?.id);
    }
  };
  ...
  useEffect(() => {
    if (router?.query?.pageMode === "Add") {
      ...
      const date = moment(user?.dateOfBirth).format("YYYY-MM-DD");
      ...
      >
        ...
        <Grid item xs={12} sm={6} md={4} lg={3} xl={2}>
          <TextField
            ...
            {...register("adddressMr")}
            error={!!errors.adddressMr}
            helperText={errors?.adddressMr ? errors.adddressMr.message : null}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={4} lg={3} xl={2}>
          <TextField
...
export default PersonalDetailsForSports;
