...
const Index = (props) => {
  ...
  const calCulateCharges = (charges) => {
    ...
    setApplicableCharages([...charges, cgstTempData, sgstTempData]);

    console.log("sdfkldsf", [...charges, cgstTempData, sgstTempData])
    setTotalAmountFinal(finalTotalWithCGSTSGST)

    console.log("32432032jdslfjdsklfjs", totalAmount, cgstTempData, sgstTempData, finalTotalWithCGSTSGST, charges)
  }



  useEffect(() => {
    console.log("applicableCharagestotalAmountFinal", applicableCharages, totalAmountFinal)
  }, [applicableCharages, totalAmountFinal]);
  ...
  useEffect(() => {
    if (router.query?.applicationId) {
      axios
        .get(`${urls.SPURL}/gymBooking/getById?id=${router.query?.applicationId}`, {
          headers: {
  ...
  return (
    <>
      <div>
        <ComponentToPrint dataa={dataa} totalAmountFinal={totalAmountFinal} applicableCharages={applicableCharages} ref={componentRef} />
      </div>
      <br />
...
class ComponentToPrint extends React.Component {
  render() {
    ...
    console.log("totalAmountFinal4324324", this?.props?.totalAmountFinal)
    console.log("applicableCharages4324324", this?.props?.applicableCharages)
    return (
      <>
                ...
                <h5 style={{ marginLeft: "40px", marginRight: "40px" }}>
                  <b>
                    ...
                    <table id="table-to-xls" className={styles.report_table}>
                      ...
                      <tbody>
                        {this?.props?.applicableCharages?.map((charge, index) => (
                          <tr key={index + 1}>
                            <td colSpan={4}>{index + 1})</td>
                            <td colSpan={4} >
                              <center>{charge?.chargeTypeName}</center>
                            </td>
                        ...
                        <tr>
                          ...
                          <td colSpan={4}>
                            <b>{this?.props?.totalAmountFinal}</b>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                    ...
                    {/* {this?.props?.dataa?.marriageDate} */}व व्यायाम शाळा
                    नोंदणीकृत आकारण्यात येणारे शुल्क
                  </b>
                  <b> एकूण फी</b> &nbsp;
                  <b>{this?.props?.sum}</b>
                  <b> रु .</b>
