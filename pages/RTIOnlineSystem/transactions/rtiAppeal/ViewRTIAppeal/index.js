...
const EntryForm = () => {
  ...
  const [applicationDetails, setRTIApplicationDetails] = useState(null);
  ...
  useEffect(() => {
    if (applicationId != null && applicationId != undefined) getLoi();
  }, [applicationId]);
  ...
  const cancellButton = () => {
    ...
    } else if (logedInUser === "departmentUser") {
      router.push({
  ...
  }, [appealDetails,statusAll, language]);
  ...
  const setAppealDetailsOnUi = () => {
    ...
    setValue(
      "address",
      language == "en" ? res.data?.address : res.data?.addressMr
    );
  ...
  const setApplicationDetailsOnForm = () => {
    ...
    // Loop through each attached document and add it to the `doc` array
    for (let i = 1; i <= 10; i++) {
      const attachedDocument = _res && _res[`attachedDocument${i}`];
      if (attachedDocument != null) {
        const DecryptPhoto = DecryptData(
          "passphraseaaaaaaaaupload",
          attachedDocument
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          {(((logedInUser == "citizenUser" ||
            ...
            (authority &&
              authority.find((val) => val == roleId.RTI_APPEALE_ROLE_ID) &&
              (statusVal === 7 || statusVal === 8 || statusVal === 11))) && (
            <>
                          ...
                          <LocalizationProvider dateAdapter={AdapterMoment}>
                            <DatePicker
                              ...
                              disabled={
                                ...
                                authority.find(
                                  (val) =>
                                    val == roleId.RTI_APPEALE_ROLE_ID &&
                  ...
                  <Grid container sx={{ padding: "10px" }}>
                    ...
                    {dataSource.length != 0 &&
                      ...
                      hearingDetails.status !== 9 && (
                        <Grid
                          item
                          xl={dataSource.length != 0 ? 4 : 6}
                          lg={dataSource.length != 0 ? 4 : 6}
                          md={dataSource.length != 0 ? 4 : 6}
                          sm={12}
                          xs={12}
                          sx={{
                            display: "flex",
