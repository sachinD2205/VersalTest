...
const Index = () => {
  ...
  const schema = yup.object().shape({
    ...
    roadTypePrefixMr: yup
    .string()
    .matches( /^[\u0900-\u097F\d]+/,
    'Must be only marathi characters/ फक्त मराठी शब्द')
    .required("रस्त्याचा प्रकार उपसर्ग आवश्यक आहे !!!"),  
    remark: yup.string().required("Road Type Prefix is required"),
    remarkMr: yup
    .string()
    .matches( /^[\u0900-\u097F\d]+/,
    'Must be only marathi characters/ फक्त मराठी शब्द')
...
// console.log("d",language);
      ...
      .then((r) => {
        ...
        let _res = result.map((r, i) => {
          ...
          return {
            ...
            roadTypeNameMr: r.roadTypeNameMr,
  ...
  const onSubmitForm = (fromData) => {
    ...
    if (btnSaveText === "Save") {
      const tempData = axios.post(`${urls.RENPURL}/mstRoadType/save`, _body,{
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      }).then((res) => {
  ...
  const deleteById = (value, _activeFlag) => {
    ...
    if (_activeFlag === "N") {
      swal({
        ...
        if (willDelete === true) {
          axios.post(`${urls.RENPURL}/mstRoadType/save`, body,{
            headers: {
              Authorization: `Bearer ${userToken}`,
            },
          }).then((res) => {
...
console.log("ds",language);
  const columns = [
    ...
    // { field: "courtNo", headerName: "Court No", flex: 1 },
    {
      ...
      field: language == "en" ? "roadTypeName" : "roadTypeNameMr",
    ...
    {
      field: language == "en" ? "remark" : "remarkMr",
              ...
              style={{
                ...
                style={{
                  ...
                  <Transliteration
                      ...
                      _key={"roadTypePrefix"}
                      labelName={"roadTypePrefix"}
                      fieldName={"roadTypePrefix"}
                      updateFieldName={"roadTypePrefixMr"}
                      sourceLang={"eng"}
                      targetLang={"mar"}
                      width={250}
                      // disabled={disabled}
                      label={<FormattedLabel id="roadTypePrefix" required />}
                      error={!!errors.roadTypePrefix}
                      helperText={
                        errors?.roadTypePrefix ? errors.roadTypePrefix.message : null
                      }
                    />
                  {/* <TextField
                ...
                style={{
                  ...
                  <TextField
                    label={<FormattedLabel id="roadTypeMr" />}
                    id="standard-basic"
                    variant="standard"
                    {...register("roadTypeNameMr")}
                    error={!!errors.roadTypeNameMr}
                    InputProps={{ style: { fontSize: 18 } }}
                    InputLabelProps={{
                      style: { fontSize: 15 },
                      //true
                      // shrink:
                      //   (watch("roadTypeNameMr") ? true : false) || (router.query.roadTypeNameMr ? true : false),
                    }}
                    helperText={errors?.roadTypeNameMr ? errors.roadTypeNameMr.message : null}
                  />
                </Grid> 
                ...
                style={{
                  ...
                  alignItems: "center",
                   ...
                   <Transliteration
                      style={{ backgroundColor: "white", margin: "250px" }}
                      _key={"roadTypePrefixMr"}
                      labelName={"roadTypePrefixMr"}
                      fieldName={"roadTypePrefixMr"}
                      updateFieldName={"roadTypePrefix"}
                      sourceLang={"mar"}
                      targetLang={"eng"}
                      width={250}
                      // disabled={disabled}
                      label={<FormattedLabel id="roadTypePrefixMr" required />}
                      error={!!errors.roadTypePrefixMr}
                      helperText={
                        errors?.roadTypePrefixMr ? errors.roadTypePrefixMr.message : null
                      }
                    />
                  {/* <TextField
