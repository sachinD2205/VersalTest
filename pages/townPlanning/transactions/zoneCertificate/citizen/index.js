...
import PersonalDetailsZoneCertificate from "../../../../../components/townPlanning/PersonalDetailsZoneCertificate";
//icon
...
import { personalDetailsSchema } from "../../../../../components/townPlanning/schema/zoneCertificate";
...
function getSteps() {
  return [
    <strong key={1}>
      ...
      {/* {language=="en"?"Zone Certificate Details":"झोन प्रमाणपत्र तपशील"}  */}
    </strong>,
    // <strong key={2}>
...
function getStepContent(step) {
  switch (step) {
    case 0:
      return <PersonalDetailsZoneCertificate key={1} />;

    // case 1:
    //   return <AddressDetailsDOP key={2} />;

    case 1:
      return <FileTable key={2} serviceId={18} />;
  ...
  const handleNext = (data) => {
    ...
    if (activeStep === steps.length - 1) {
      if (router?.query?.pageMode != "View") {
        axios
          .post(`${urls.TPURL}/transaction/zoneCertificate/save`, finalBody, {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
          })
    ...
    <>
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            <h2>{language == "en" ? "ZONE CERTIFICATE" : "झोन प्रमाणपत्र"}</h2>
          </Grid>
          <Stepper
            alternativeLabel
