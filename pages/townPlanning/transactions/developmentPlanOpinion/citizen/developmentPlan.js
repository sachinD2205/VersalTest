...
//personal d
import PersonalDetailsDOP from "../../../../../components/townPlanning/PersonalDetailsDOP";
//icon
...
} from "../../../../../components/townPlanning/schema/developmentPlan";
...
function getStepContent(step) {
  switch (step) {
    case 0:
      return <PersonalDetailsDOP key={1} />;
    case 1:
      return <FileTable key={2} serviceId={19} />;
...
const Index = () => {
  ...
  useEffect(() => {
    if (router.query.pageMode == "Edit" || router.query.pageMode == "View") {
      ...
      axios
        .get(
          `${urls.TPURL}/developmentPlanOpinion/getDevelopmentPlanOpinion/${router?.query?.id}`,
          {
            headers: {
        ...
        .then((resp) => {
          ...
          let fileee = watch("files");
          console.log("development plan opp", fileee);
        })
        .catch((error) => {
  ...
  const handleNext = (data) => {
    ...
    console.log("development plan files", watch("files"));
    const finalBody = {
      ...data,
    ...
    if (activeStep === steps.length - 1) {
      if (router?.query?.pageMode != "View") {
        axios
          .post(`${urls.TPURL}/developmentPlanOpinion/save`, finalBody, {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
          })
          .then((res) => {
            if (res.status == 200) {
              ...
              //     `${
              //       urls.TPURL
              //     }/developmentPlanOpinion/getDevelopmentPlanOpinion/${
    ...
    <>
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          ...
          >
            <h2>
              {language == "en" ? "DEVELOPMENT PLAN OPINION" : "विकास योजना मत"}
            </h2>
          </Grid>
          <Stepper
            alternativeLabel
