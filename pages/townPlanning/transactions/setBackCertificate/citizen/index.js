...
import PersonalDetailssetback from "../../../../../components/townPlanning/PersonalDetailssetback";
//icon
...
import { personalDetailsSchema } from "../../../../../components/townPlanning/schema/setBackCertificate";
...
function getSteps() {
  return [
    <strong key={1}>
      <FormattedLabel id="setBackCertificateDetails" />
      {/* Set Back Certificate Details */}
    </strong>,
    // <strong key={2}>
...
function getStepContent(step, pageMode) {
  switch (step) {
    case 0:
      return <PersonalDetailssetback key={1} />;

    // case 1:
    //   return <AddressDetailsDOP key={2} />;

    case 1: {
      // return <FileTable key={2} serviceId={20} />;}

      if (pageMode == "View") {
        console.log("ssssssssssss");
        return <fileTableVerification />;
      } else {
        return <FileTable key={2} serviceId={20} />;
      }
    }
...
const Index = () => {
  ...
  const handleNext = (data) => {
    ...
    if (activeStep == steps.length - 1) {
      axios
        .post(`${urls.TPURL}/setBackCertificate/save`, finalBody, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
        .then((res) => {
          if (res.status == 200) {
            ...
            // axios
            //   .get(
            //     `${urls.TPURL}/setBackCertificate/getById/${res?.data?.message}`,{
            //       headers: {
            //         Authorization: `Bearer ${user.token}`,
            //       },
            //     }
          ...
          {activeStep === steps.length ? (
            ...
            <FormProvider {...methods}>
              <form onSubmit={methods.handleSubmit(handleNext)}>
                {getStepContent(activeStep, router?.query?.pageMode)}
